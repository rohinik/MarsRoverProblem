...
class ProcessInput {
    static setTheRover(String roverCoordinates) {
        ...
        roversPoint.setLocation(Integer.parseInt(roverVariables[0]), Integer.parseInt(roverVariables[1]))
        if (!MarsRoverConstants.DIRECTION_INSTANCE_LIST.containsKey(roverVariables[2].toUpperCase())) {
            throw new RoversException("Wrong roverDirection (N,S,E,W)\"");
        } else {
            direction = MarsRoverConstants.DIRECTION_INSTANCE_LIST.get(roverVariables[2].toUpperCase())
        }
        new Rover(position: roversPoint, roverDirection: direction)
    }
}
